    ...
    tools=[
        {
            "function_declarations": [
                ...
                {
                    "name": "render_eclipse",
                    "description": "Only if the user asks how does an eclipse work, not eclipse glasses, render a 3d model of the eclipse",
                    "parameters": {
                        "type": "object",
                        "properties": {
                            "object": {
...
async def receive_data():
    ...
    if "help" in user_input:
        prompt = user_input + ". According to Gemini Pro Vision Analysis: user is looking at a Cooler Master 212 EVO CPU Cooler."
    elif "these" in user_input:
        prompt = user_input + ". According to Gemini Pro Vision Analysis: user is looking at a pair of eclipse glasses."
        return (
            jsonify(
                {
                    "status": "success",
                    "type": "text",
                    "text": """These are special glasses designed for viewing solar eclipses. They are called eclipse glasses or solar viewers.
The lenses of these glasses are made of a very dark material that filters out most of the sunlight, making it safe to look directly at the sun during an eclipse. It's important to use proper eye protection like these glasses during a solar eclipse because looking directly at the sun without protection can cause serious eye damage.""",
                    "image": imageString,
                }
            ),
            200,
